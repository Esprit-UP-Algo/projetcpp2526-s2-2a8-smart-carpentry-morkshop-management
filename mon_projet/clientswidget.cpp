...
void ClientsWidget::chargerTable(QSqlQuery query)
{
    ...
    while (query.next()) {
        ...
        ui->tableWidgetClients->setItem(row, 3, new QTableWidgetItem(query.value("TELEPHONE").toString()));
        ui->tableWidgetClients->setItem(row, 4, new QTableWidgetItem(query.value("EMAIL").toString()));
        ui->tableWidgetClients->setItem(row, 5, new QTableWidgetItem(query.value("ADDRESSE").toString()));
        row++;
    }
}
...
void ClientsWidget::onModifier()
{
    ...
    if (c.modifier()) {
        ...
        QMessageBox::warning(this, "Erreur SQL", "Échec de la modification. Vérifiez les données.");
    }
}
...
void ClientsWidget::onSupprimer()
{
    ...
    if (Client::supprimer(id)) {
        ...
        QMessageBox::warning(this, "Erreur SQL", "Échec de la suppression. Vérifiez les données.");
    }
}
