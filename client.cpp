...
bool Client::ajouter() const
{
    ...
    query.prepare(
        ...
        "(NOM, PRENOM, TELEPHONE, EMAIL, ADDRESSE, SOCIETE, MATRICULE_FISCALE, N_CIN, SITE_WEB_FB) "
        "VALUES (:nom, :prenom, :tel, :email, :adresse, :societe, :mf, :ncin, :siteweb)"
    );
    ...
    if (!ok) {
        ...
        qDebug() << "Client::ajouter error:" << lastSqlError;
    }
    return ok;
}
...
bool Client::modifier() const
{
    ...
    if (!ok) {
        lastSqlError = query.lastError().text();
        qDebug() << "Client::modifier error:" << lastSqlError;
    }
    return ok;
}
...
bool Client::supprimer(const QString &id)
{
    ...
    query.bindValue(":id", id.toInt());
    bool ok = query.exec();
    if (!ok) qDebug() << "Client::supprimer error:" << query.lastError().text();
    return ok;
}
